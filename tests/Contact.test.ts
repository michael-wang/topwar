...
const tuning: SimulationTuning = {
  ...
  rifle: { damage: 3, tier2DamageMultiplier: 100, tier3DamageMultiplier: 1000, fireRate: 7, projectileSpeed: 10, range: 18 },
...
describe('Enemy contact casualties', () => {
  it('preserves rocket specialists until rifle soldiers are gone', () => {
    ...
    expect(oneContact.getState().squad).toEqual({ count: 1, rocketCount: 1, tier2RifleCount: 0, tier3RifleCount: 0 });
    const twoContacts = simulationWith(2, [grunt(1, 0, 0), grunt(2, 0, 0)], [], 1);
    twoContacts.step(0.1, { targetX: 0 }, tuning);
    expect(twoContacts.getState().squad).toEqual({ count: 0, rocketCount: 0, tier2RifleCount: 0, tier3RifleCount: 0 });
  });
