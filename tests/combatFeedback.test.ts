...
describe('visual-only squad damage feedback', () => {
  it('counts Tier-1, Tier-2, and rocket defensive value', () => {
    expect(squadDefenseValue({ count: 1, tier2RifleCount: 0, rocketCount: 0 })).toBe(1);
    expect(squadDefenseValue({ count: 1, tier2RifleCount: 1, rocketCount: 0 })).toBe(10);
    expect(squadDefenseValue({ count: 9, tier2RifleCount: 0, rocketCount: 0 })).toBe(9);
    expect(squadDefenseValue({ count: 5, tier2RifleCount: 2, rocketCount: 1 })).toBe(23);
  });
