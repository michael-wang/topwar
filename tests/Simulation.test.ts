...
const create = () => new Simulation({ seed: 1, level, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000 });
...
describe('Simulation', () => {
  ...
  it('accepts a zero-soldier start and rejects invalid squad counts', () => {
    expect(new Simulation({ seed: 1, level, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000}).getState().squad.count).toBe(0);
    for (const startSquad of [-1, 1.5, Number.MAX_SAFE_INTEGER + 1, Number.NaN, Infinity]) {
      expect(() => new Simulation({ seed: 1, level, startSquad, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000})).toThrow(/startSquad/);
    }
  });
  ...
  it('produces identical state for the same seed, level, and step sequence', () => {
    ...
    const first = new Simulation({ seed: 0xffffffff, level: repeatLevel, startSquad: 3, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000 });
    const second = new Simulation({ seed: 0xffffffff, level: repeatLevel, startSquad: 3, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000 });
  ...
  it('returns owned copies and restores a JSON round-trip including RNG state', () => {
    ...
    const another = new Simulation({ seed: 99, level: { ...level, id: 'other' }, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000 });
  ...
  it.each([-1, 1.5, 4294967296, Number.NaN, Infinity])('rejects invalid seed %s', (seed) => {
    expect(() => new Simulation({ seed, level, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000})).toThrow();
  });

  it.each(['', '   ', null, 7])('rejects invalid level id %s', (levelId) => {
    expect(() => new Simulation({ seed: 1, level: { ...level, id: levelId as string }, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000})).toThrow();
  });
  ...
  it.each([
    ...
    (state: SimulationState) => { state.squad.rocketCount = -1; },
    (state: SimulationState) => { state.squad.rocketCount = 1.5; },
    (state: SimulationState) => { state.squad.rocketCount = state.squad.count + 1; },
    (state: SimulationState) => { delete (state.squad as unknown as Record<string, unknown>).rocketCount; },
...
describe('Static authored enemies', () => {
  ...
  const authored = { ...parsedAuthored,
    enemyStream: { ...parsedAuthored.enemyStream!, bosses: undefined } };
  ...
  it('materializes the same initial enemy horizon on every fresh run', () => {
    ...
    expect(first.enemyStream).toEqual({ nextRowIndex: 121, nextEnemyId: 848,
      ...
      nextRewardId: first.streamRewards.length + 1, nextBossIndex: 0 });
  ...
  it('restores enemy data after JSON serialization', () => {
    ...
    const restored = new Simulation({ seed: 99, level: authored, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300, tier3Hp: 3000 });
