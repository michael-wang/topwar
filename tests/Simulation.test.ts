...
describe('Simulation', () => {
  ...
  it('accepts a zero-soldier start and rejects invalid squad counts', () => {
    expect(new Simulation({ seed: 1, level, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300 }).getState().squad.count).toBe(0);
    for (const startSquad of [-1, 1.5, Number.MAX_SAFE_INTEGER + 1, Number.NaN, Infinity]) {
      expect(() => new Simulation({ seed: 1, level, startSquad, startRocketCount: 0, gruntHp: 10, bruteHp: 300 })).toThrow(/startSquad/);
    }
  });
  ...
  it('produces identical state for the same seed, level, and step sequence', () => {
    ...
    const first = new Simulation({ seed: 0xffffffff, level: repeatLevel, startSquad: 3, startRocketCount: 0, gruntHp: 10, bruteHp: 300 });
    const second = new Simulation({ seed: 0xffffffff, level: repeatLevel, startSquad: 3, startRocketCount: 0, gruntHp: 10, bruteHp: 300 });
  ...
  it('returns owned copies and restores a JSON round-trip including RNG state', () => {
    ...
    const another = new Simulation({ seed: 99, level: { ...level, id: 'other' }, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300 });
  ...
  it.each([-1, 1.5, 4294967296, Number.NaN, Infinity])('rejects invalid seed %s', (seed) => {
    expect(() => new Simulation({ seed, level, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300 })).toThrow();
  });

  it.each(['', '   ', null, 7])('rejects invalid level id %s', (levelId) => {
    expect(() => new Simulation({ seed: 1, level: { ...level, id: levelId as string }, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300 })).toThrow();
  });
...
describe('Static authored enemies', () => {
  ...
  const createAuthored = () => new Simulation({ seed: 1, level: authored, startSquad: 1, startRocketCount: 0, gruntHp: 10, bruteHp: 300 });
  ...
  it('restores enemy data after JSON serialization', () => {
    ...
    const restored = new Simulation({ seed: 99, level: authored, startSquad: 0, startRocketCount: 0, gruntHp: 10, bruteHp: 300 });
