...
Level 001 uses an endless deterministic spatial enemy stream. New rows are generated only ahead of the advancing player; grunts stay stationary, and uncleared enemies cause contact or defense-line casualties until the run ends. The stream is not a huge preallocated enemy list and may continue beyond the level's current length metadata. Rare deterministic +1 reward targets appear beside intact enemy rows, asking the player to steer and divert fire without reducing enemy pressure. Reward opportunities remain sparse in the opening and become more frequent as Tier-2 pressure rises, but pursuing them remains an optional risk.
...
Level 001 no longer uses permanent side armories as its active reward economy. A deterministic row roll adds a stationary +1 reward target at the left or right edge while all seven enemies remain. Pursuing it trades movement and firepower against survival. Reward frequency rises from 2.5% to 25% per row with Tier-2 enemy pressure. Tier-2 rewards can appear during mixed rows, with their chance weighted by the 10:1 tier exchange value. This keeps growth mathematically possible while collecting rewards still requires player skill. It takes 10 rifle hits at or above the reward tier to unlock and immediately add one soldier; projectile damage does not change progress. Ignored targets expire harmlessly behind the defense line. Generic side-armory code remains available for custom levels but is inactive in Level 001.
